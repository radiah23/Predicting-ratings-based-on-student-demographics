...
The horizontal bar chart of the ten districts with the highest average percent economically disadvantaged highlights a small group of districts and charter schools with very high concentrations of economically disadvantaged students.This shows Schools with 100% or near 100% are a mix between charter and public alternative schools. Additionally, 9 out of 10 of these schools are also within highly metropolitan areas.
...
$$
\text{overall score}_i = \beta_0 + \beta_1 \cdot \text{academic growth score}_i + \beta_2 \cdot \text{distinction score}_i + \beta_3 \cdot \text{school progress score}_i$$ $$+ \beta_4 \cdot \text{percent economically disadvantaged}_i + \beta_5 \cdot \text{number of students}_i + \varepsilon_i 
$$
...
To determine the model, we started with calculating the VIF to detect any multicolinearity issues among the explanatory variables. We calculated this using both the reduced and full model. There were no multicolinearity issues given all of the VIFs show litle colinearity < 5.

We began to test our model using forward model selection with AIC. See Appendix \@ref(app-aic)) for detailed results. 



# Appendix 
## Appendix A: Conditions of the linear regression model {#app-conditions}
## Appendix B: VIF {#app-vif}
## Appendix C: Stepwise AIC {#app-aic}
## Appendix D: Influential Points {#app-influential}
