...
```{r}
library(broom)
full_augmented <- augment(full_model)
  full_augmented$campus <- final_df$campus
  select(.cooksd, .std.resid, .hat, .sigma, overall_score, academic_growth_score, distinction_score, school_progress_score,overall_score,percent_economically_disadvantaged, number_of_students ) 
full_augmented
```
...
leverage_threshold <- 2 * (6)/1356
leverage_threshold

```
```{r}
library(ggplot2)
ggplot(full_augmented, aes(y = .hat)) +
  geom_point() +
geom_hline(yintercept = 0.008849558,
             color = "blue") +  
  coord_flip()+
  theme_minimal()
```
